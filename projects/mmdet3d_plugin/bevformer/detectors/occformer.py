...
class OccFormer(MVXTwoStageDetector):
    ...
    def forward_test(self,  img_metas,
        ...
        if DEBUG:
            ...
            use_mask=False
            if use_mask:
                save_root = '/home/txy/occ_nus_out/use_mask/'
            else:
                save_root = '/home/txy/occ_nus_out/'
            sample_idx=img_metas[0][0]['sample_idx']
            np.savez_compressed(save_root+sample_idx,output=occ_results.to(torch.uint8).cpu().numpy(),
                                gt=voxel_semantics[0].to(torch.uint8).cpu().numpy())
            print('saved',save_root+sample_idx)
