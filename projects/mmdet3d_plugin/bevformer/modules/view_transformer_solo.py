...
class ViewTransformerSOLOFusion(ViewTransformerLiftSplatShoot_solo):
    def __init__(self, 
                 extra_depth_net, 
                 loss_depth_weight, 
                 se_config=dict(), 
    ...
    def get_mono_depth(self, curr_sem_feats, rots, trans, intrins, post_rots, post_trans):
        ...
        # print("curr_sem_feats.dtype", curr_sem_feats.dtype)
        # print("rots.dtype", rots.dtype)
        # print("rots.dtype", rots.dtype)
        # print("intrins.dtype", intrins.dtype)
        # print("post_rots.dtype", post_rots.dtype)
        # print("post_trans.dtype", post_trans.dtype)
    ...
    def forward(self, 
        ...
        if not self.do_history_stereo_fusion or prev_stereo_feats is None:
            assert curr_stereo_feats is None
            depth_digit = mono_depth_digit
        else:
        # if self.do_history_stereo_fusion and prev_stereo_feats is not None:
            # Get stereo fusion feature
            ...
            with torch.no_grad():
                ...
                for prev_cam_idx in range(N):
                    ...
                    curr_prev_sampled_feats_padded = F.grid_sample(curr_prev_stereo_feats,
                                                                curr_prev_meshgrid_xy_norm_valid_padded.unsqueeze(2), align_corners=False) # B x C x max_length x 1
