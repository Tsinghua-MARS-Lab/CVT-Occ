...
class BEVFormerEncoder3D(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, num_points_in_voxel=2,num_voxel=8, return_intermediate=False, dataset_type='nuscenes',
                 **kwargs):

        super(BEVFormerEncoder3D, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate

        self.num_points_in_pillar = num_points_in_voxel
        self.n_p_in_voxel=num_voxel
    ...
    def get_reference_points(H, W, Z=8, n_p_in_voxel=2,n_voxel=8, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        """
        n_p_in_pillar=n_voxel*n_voxel
        # reference points in 3D space, used in spatial cross-attention (SCA)
        if dim == '3d':
            zs = torch.linspace(0.2, Z - 0.2, n_p_in_pillar, dtype=dtype,
                                device=device).view(n_voxel,n_p_in_pillar, 1, 1).permute(1,0,2,3).expand(n_p_in_voxel,n_voxel, H, W)  / Z
