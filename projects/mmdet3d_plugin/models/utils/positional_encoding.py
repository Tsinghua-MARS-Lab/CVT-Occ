...
@POSITIONAL_ENCODING.register_module()
class LearnedPositionalEncoding3DAdd(BaseModule):
    """Position embedding with learnable embedding weights.
    ...
    def __init__(self,
                 ...
                 height_num_embed=16,
                 init_cfg=dict(type='Uniform', layer='Embedding')):
        super(LearnedPositionalEncoding3DAdd, self).__init__(init_cfg)
        self.num_feats = num_feats
    ...
    def forward(self, mask):
        ...
        #         1, w, 1)),
        #     dim=-1).permute(2, 0,
        #                     1).unsqueeze(0).repeat(mask.shape[0], 1, 1, 1)   # (bs, embed_dims, h, w)
